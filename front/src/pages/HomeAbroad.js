    ...
    const MatePosts = async () => {
      ...
      try {
        ...
        for (const post of topThreePosts) {
          const res = await axios.get(`/api/v1/posts/${post.id}`) // 각 게시물의 ID로 작성자 프로필 정보 요청
          profiles[post.id] = res.data.userProfileInfo // 프로필 정보를 저장
        }
        setWriterProfiles(profiles) // 모든 프로필 정보
      } catch (error) {
    ...
    <div className="container mx-auto px-8 bg-white min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div className="flex items-center justify-between mb-8 p-4 bg-tripDuoGreen rounded-lg shadow-lg">
          <h3 className="text-xl font-semibold text-white">해외 여행메이트 찾기</h3>
        ...
        <div className="flex items-center justify-between mb-8 p-4 bg-tripDuoGreen rounded-lg shadow-lg">
          <h3 className="text-xl font-semibold text-white">해외 여행 코스 검색</h3>
          <div className="flex space-x-4">
          ...
          {matePosts.map((post) => (
            ...
            >
              ...
              <div className="flex justify-center -mt-16">
                {writerProfiles[post.id]?.profilePicture ? ( // 각 게시물의 작성자 프로필 사진 표시
                  <img src={writerProfiles[post.id].profilePicture} className="w-20 h-20 rounded-full" alt="" />
                ) : (
                  <svg
